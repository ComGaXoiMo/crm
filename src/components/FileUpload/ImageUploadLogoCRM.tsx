...
@observer
class ImageUploadLogoCRM extends AppComponentListBase<IImageUploadWrapProps> {
  state = {
    files: [] as any[],
  ...
  initFiles = async () => {
    ...
    const listFiles = files.filter(
      (item) => item.documentTypeId === documentType.logo
    )
  ...
  handleBeforeUploadFile = (file) => {
    ...
    reader.onload = (e) => {
      ...
      image.onload = () => {
        ...
        const maxHeight = 150 // Set your maximum height

        if (image.width > maxWidth || image.height > maxHeight) {
          message.error(
            `Image dimensions should be within ${maxWidth}x${maxHeight} pixels!`
          )
        } else {
          const params = {
  ...
  render() {
    return (
      <div>
        ...
        >
          {this.state.files.length >= 1 ? null : this.uploadButton}
        </Upload>
        <Modal
...
export default withRouter(ImageUploadLogoCRM)
