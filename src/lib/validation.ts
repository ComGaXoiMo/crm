...
export function checkArrayPhone(rule, value, label,canNull?) {
  if (!value || !value.length) {
    return Promise.resolve()
  }
  if(canNull && value[0].phone ===""){
    return Promise.resolve()
  }
  if (
    value.findIndex(
      (phone) => isNullOrEmpty(phone.phone) || !phoneRegex.test(phone.phone)
    ) !== -1
  ) {
...
export function checkArrayEmail(rule, value, label) {
  ...
  if (value.findIndex((email) => isNullOrEmpty(email.email)) !== -1) {
    return Promise.resolve()
  } else if (value.findIndex((email) => !emailRegex.test(email.email)) !== -1) {
    return Promise.reject(LError("INVALID_FORMAT_{0}", L(label)))
  }
  return Promise.resolve()
}
...
export function checkArrayOpportunityDepartment(rule, value, label) {
  ...
  if (
    value.findIndex(
      (company) =>
        !company.organizationUnitId ||
        !company.instructionId ||
        company.feeAmount < 0
    ) !== -1
  ) {
    return Promise.reject(LError("PLEASE_SELECT_{0}", L(label)))
...
export function checkArrayCommercialOpportunityDepartment(rule, value, label) {
  ...
  if (
    value.findIndex(
      (company) => !company.organizationUnitId || company.feeAmount < 0
    ) !== -1
  ) {
    return Promise.reject(LError("PLEASE_SELECT_{0}", L(label)))
  }
