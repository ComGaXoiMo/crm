...
@observer
class CardAverageRentalRate extends AppComponentListBase<IOverviewProps> {
          ...
          <Row gutter={[4, 4]}>
            ...
            <Col sm={{ span: 24 }}>
              <span className="card-overview-content">
                {this.L("VND")}
                <strong>{formatNumber(data?.ThisAvgAmount)}</strong>
              </span>
            </Col>
            <Col sm={{ span: 24 }}>
              <Row gutter={[24, 4]}>
                ...
                <Col sm={{ span: 6 }} className="card-text-strong">
                  {this.L("AVG_RENTAL_FEE")}
                </Col>
                <Col sm={{ span: 6 }} className="card-text-right">
                  {" "}
                  {formatNumber(data?.LastAvgAmount)}
                </Col>
                <Col sm={{ span: 6 }} className="card-text-right">
                  {formatNumber(data?.ThisAvgAmount)}
                </Col>
                <Col
                  sm={{ span: 6 }}
                  className={`card-text-strong ${
                    data?.DiffAvgAmount >= 0
                      ? "card-text-green"
                      : "card-text-red"
                  } `}
                >
                  {formatNumberWithSignal(data?.DiffAvgAmount)}
                  <br />(
                  {formatNumberFloat((data?.PercentAvgAmount ?? 0) * 100)}
                  %)
                </Col>
              </Row>
            </Col>
...
export default withRouter(CardAverageRentalRate)
