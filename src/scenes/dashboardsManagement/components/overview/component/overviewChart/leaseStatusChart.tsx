...
const LeaseStatusChart = (props: Props) => {
  React.useEffect(() => {
    const res = props.data.map((item) => `${item.LAStageName}`)
    setLabels(res)
  ...
  const getListKey = (lsitData) => {
    ...
    setListKey(["LastNumCount", "ThisNumCount"])
    // setListKey(
    //   statusKeysArray.filter(
    //     (item) => item === "LastNumCount" || item === "ThisNumCount"
    //   )
    // )
  }
    ...
    datasets: listKey.map((item, index) => {
      return {
        ...
        data: labels.map((label) => {
          const res = props.data.find((data) => `${data.LAStageName}` === label)
          return res?.[item] ?? 0
        }),
  ...
  return (
    <>
      <Card className="card-report">
        <span className="card-overview-title">{L("LEASE_STATUS")}</span>
        <Row gutter={[8, 8]}>
...
export default LeaseStatusChart
