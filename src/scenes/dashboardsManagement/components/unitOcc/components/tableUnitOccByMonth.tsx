...
@observer
class TableUnitOccByMonth extends AppComponentListBase<IProps> {
  ...
  getDashBoard = async () => {
    this.props.dashboardStore
      ...
      .finally(() => {
        this.dataAnalysis(
          this.props.dashboardStore.dashboardUnitOccDetailsMonthData
        );
        this.setState({
          dataTable: this.props.dashboardStore.dashboardUnitOccDetailsMonthData,
        });
      });
  };
        ...
        <thead>
          <tr>
            ...
            {colArr.map((colData, index) => {
              return (
                <th>{moment(colData?.colName).format(monthCharFormat)}</th>
              );
            })}
            <th>{L("AVG_OCC")}</th>
          </tr>
        ...
        <Spin spinning={this.props.dashboardStore.isLoading}>
          <div ref={this.printRef} className="dashboard-style">
            <Row gutter={[8, 8]}>
              ...
              <Col sm={{ span: 24, offset: 0 }}>
                <div className="table-contr" id="tblOccUnitByMonth">
                  {this.dataAnalysis(
                    this.props.dashboardStore.dashboardUnitOccDetailsMonthData
                  )}
                </div>
...
export default withRouter(TableUnitOccByMonth);
