...
@observer
class TableOccSummaryByUnitType extends AppComponentListBase<IProps> {
  ...
  getDashBoard = async () => {
    this.props.dashboardStore
      .getDashboardSummaryOccUnitType({
        ...this.state.filters,
      })
      .finally(() => {
        this.dataAnalysis(
          this.props.dashboardStore.dashboardOccSummaryUnitTypeData
        )
        this.setState({
          dataTable: this.props.dashboardStore.dashboardOccSummaryUnitTypeData,
        })
      })
  }
    ...
    return (
      <table className="table-occ-summary">
        ...
        <tbody>
          ...
          <tr>
            <td className="text-red-hilight">{L("TOTAL_UNIT")}</td>
            {dataInput.map((colData, index) => {
              return <td> {colData?.TotalUnit}</td>
            })}
            <td>{dataInput?.reduce((n, { TotalUnit }) => n + TotalUnit, 0)}</td>
          </tr>
        </tbody>
      </table>
    )
        ...
        <Spin spinning={this.props.dashboardStore.isLoading}>
          <div ref={this.printRef} className="dashboard-style">
            <Row gutter={[8, 8]}>
              ...
              <Col sm={{ span: 24, offset: 0 }}>
                <div className="table-contr" id="tblOccSummaryUnitType">
                  {this.dataAnalysis(
                    this.props.dashboardStore.dashboardOccSummaryUnitTypeData
                  )}
                </div>
...
export default withRouter(TableOccSummaryByUnitType)
