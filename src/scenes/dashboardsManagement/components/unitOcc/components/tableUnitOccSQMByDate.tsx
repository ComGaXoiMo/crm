...
@observer
class TableUnitOccSQMByDate extends AppComponentListBase<IProps> {
  printRef: any = React.createRef()
  state = {
    filters: {
      type: dashboardOccType.date,
  ...
  getDashBoard = async () => {
    this.props.dashboardStore
      .getDashboardUnitOccSQM({
        ...this.state.filters,
      })
      .finally(() => {
        this.dataAnalysis(
          this.props.dashboardStore.dashboardUnitOccSQMDetailsDateData
        )
        this.setState({
          dataTable:
            this.props.dashboardStore.dashboardUnitOccSQMDetailsDateData,
        ...
        <thead>
          <tr>
            ...
            {colArr.map((colData, index) => {
              return <th key={`th${index}`}>D{index + 1}</th>
            })}
            <th>{L("AVG_OCC")}</th>
          </tr>
        </thead>
            ...
            return (
              <>
                ...
                <tr>
                  ...
                  {project?.dataProject.map((item) => {
                    const thisCell = item?.Percent - minOcc
                    const colorRank =
                      (thisCell / (rageOccPercent > 0 ? rageOccPercent : 1)) *
                      100
                    const colorCell = `rgba(7, 143, 233, ${
                      Math.round(colorRank) / 100
                    })`
                    return (
                      <td
                ...
                <tr>
                  <td>{L("TOTAL_UNIT_SQM")}</td>
                  {project?.dataProject.map((item) => {
                    return (
                      <td className="text-right">
                        {formatNumberFloat(item?.TotalSQM)}
                      </td>
                    )
                  })}
                  <td></td>
        ...
        <Spin spinning={this.props.dashboardStore.isLoading}>
          <div ref={this.printRef} className="dashboard-style">
            <Row gutter={[8, 8]}>
              ...
              <Col sm={{ span: 24, offset: 0 }}>
                <div className="table-contr" id="tblOccUnitByDate">
                  {this.dataAnalysis(
                    this.props.dashboardStore.dashboardUnitOccSQMDetailsDateData
                  )}
                </div>
...
export default withRouter(TableUnitOccSQMByDate)
