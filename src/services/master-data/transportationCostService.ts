...
class TransportationCostService {
  ...
  public async activateOrDeactivate(ids: number, isActive) {
    const response = await http.post('api/services/app/Transportation/MultiActives', ids, { params: { isActive } })
    notifySuccess(LNotification('SUCCESS'), LNotification('SAVING_SUCCESSFULLY'))
    return response.data
  }

  public async delete(id: number, isActive) {
    const response = await http.delete('api/services/app/Transportation/Delete', {
      params: { isActive }
    })
    return response.data
  }
  ...
  public async downloadTemplateImport() {
    const response = await http.get('api/Imports/Transportation/GetTemplate', { responseType: 'blob' })
  ...
  public async importByTemplateImport(file) {
    ...
    const response = await http.post('api/Imports/Transportation/ImportFromExcel', formData, config)
    return response.data
  }
