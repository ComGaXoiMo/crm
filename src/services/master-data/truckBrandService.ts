...
class TruckBrandService {
  ...
  public async filter(params: any): Promise<PagedResultDto<any>> {
    const res = await http.get('api/services/app/TruckBrands/GetAll', { params })
  ...
  public async getAll(params: any): Promise<TruckBrandModel[]> {
    const res = await http.get('api/services/app/TruckBrands/GetLists', { params })
    const result = res.data.result
    result.items = TruckBrandModel.assigns(result.items || [])
    return result
  }
  ...
  public async getById(id): Promise<any> {
    ...
    const result = await http.get('api/services/app/TruckBrands/Get', { params: { id } })
    return TruckBrandModel.assign(result.data.result)
  }

  public async activateOrDeactivate(ids: number[], isActive) {
    const response = await http.post('api/services/app/TruckBrands/MultiActives', ids, { params: { isActive } })
    notifySuccess(LNotification('SUCCESS'), LNotification('SAVING_SUCCESSFULLY'))
    return response.data
  }

  public async delete(id: number, isActive) {
    const response = await http.delete('api/services/app/TruckBrands/Delete', { params: { id, isActive } })
    return response.data
  }

  public async downloadTemplateImport() {
    const response = await http.get('api/Imports/TruckBrands​/GetTemplateImport', { responseType: 'blob' })
  ...
  public async importByTemplateImport(file) {
    ...
    const response = await http.post('api/Imports/TruckBrands​/ImportFromExcel', formData, config)
    return response.data
  }

  public async exportTruckBrand(params: any): Promise<any> {
    const res = await http.get('/api/Export/ExportTruckBrand', { params, responseType: 'blob' })
