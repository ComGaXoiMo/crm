...
class TruckTypeService {
  ...
  public async getAll(params: any): Promise<TruckTypeModel[]> {
    const res = await http.get('api/services/app/TruckTypes/GetLists', { params })
    notifySuccess(LNotification('SUCCESS'), LNotification(L('SAVE_SUCCESSFULLY')))
    const result = res.data.result
    result.items = TruckTypeModel.assigns(result.items || [])
    return result
  ...
  public async getById(id): Promise<any> {
    ...
    const result = await http.get('api/services/app/TruckTypes/Get', { params: { id } })
    return TruckTypeModel.assign(result.data.result)
  }

  public async activateOrDeactivate(ids: number, isActive) {
    const response = await http.post('api/services/app/TruckTypes/MultiActives', ids, { params: { isActive } })
    notifySuccess(LNotification('SUCCESS'), LNotification('SAVING_SUCCESSFULLY'))
    return response.data
  }

  public async delete(id: number, isActive) {
    const response = await http.delete('api/services/app/TruckTypes/Delete', { params: { id, isActive } })
    return response.data
  }

  public async downloadTemplateImport() {
    const response = await http.get('api/Imports/TruckTypes/GetTemplateImport', { responseType: 'blob' })
  ...
  public async importByTemplateImport(file) {
    ...
    const response = await http.post('api/Imports/TruckTypes/ImportFromExcel', formData, config)
    return response.data
  }

  public async exportTruckType(params: any): Promise<any> {
    const res = await http.get('/api/Export/ExportTruckType', { params, responseType: 'blob' })
