...
class RatingBadgeService {
  ...
  public async filter(params: any): Promise<PagedResultDto<any>> {
    const res = await http.get('api/services/app/WorkflowTracker/GetAll', { params })
  ...
  public async getAll(params: any): Promise<RatingBadgeModel[]> {
    const res = await http.get('api/services/app/WorkflowTracker/GetList', { params })
    const result = res.data.result
  ...
  public async getById(id): Promise<any> {
    ...
    const result = await http.get('/api/services/app/WorkflowTracker/GetForEdit', { params: { id } })
    return RatingBadgeModel.assign(result.data.result)
  }

  public async activateOrDeactivate(ids: number, isActive) {
    const response = await http.post('api/services/app/WorkflowTracker/MultiActives', ids, { params: { isActive } })
    notifySuccess(LNotification('SUCCESS'), LNotification('SAVING_SUCCESSFULLY'))
    return response.data
  }

  public async delete(id: number) {
    const response = await http.delete('api/services/app/WorkflowTracker/Delete', { params: { id } })
    return response.data
  }

  public async downloadTemplateImport() {
    const response = await http.get('api/Imports/WorkflowTracker/GetTemplateImport', { responseType: 'blob' })
  ...
  public async importByTemplateImport(file) {
    ...
    const response = await http.post('api/Imports/CarCompany/ImportFromExcel', formData, config)
    return response.data
  }
